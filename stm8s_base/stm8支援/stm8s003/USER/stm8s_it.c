...
/**
 ...
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
 {
   ...
   if(UART1_GetITStatus(UART1_IT_RXNE)!=RESET)
   {
     ...
     if((UART1_RX_NUM&0X80)==0)
     {
        if(UART1_RX_NUM&0X40)//收到0x0d
        {
            if(Res!=0x0a) UART1_RX_NUM=0;
            else UART1_RX_NUM|=0X80;
        }
        else
        {
          if(Res==0x0d) UART1_RX_NUM|=0x40;
          else
          {
            RxBuffer1[UART1_RX_NUM&0x3f]=Res;
            UART1_RX_NUM++;
            if(UART1_RX_NUM>63) UART1_RX_NUM=0;//接收数据错误,重新开始接收  
          }
        }
     }
   }
