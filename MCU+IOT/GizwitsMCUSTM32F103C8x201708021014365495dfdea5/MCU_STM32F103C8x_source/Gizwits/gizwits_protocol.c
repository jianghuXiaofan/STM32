...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagRgbLed_OnOff)
    {
        info->event[info->num] = EVENT_RGBLED_ONOFF;
        info->num++;
        dataPoints->valueRgbLed_OnOff = gizDecompressionValue(RGBLED_ONOFF_BYTEOFFSET,RGBLED_ONOFF_BITOFFSET,RGBLED_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagRed_Led)
    {
        info->event[info->num] = EVENT_RED_LED;
        info->num++;
        dataPoints->valueRed_Led = gizX2Y(RED_LED_RATIO,  RED_LED_ADDITION, issuedData->attrVals.valueRed_Led); 
    }
    ...
    if(0x01 == issuedData->attrFlags.flagGreen_Led)
    {
        ...
        info->num++;
        dataPoints->valueGreen_Led = gizX2Y(GREEN_LED_RATIO,  GREEN_LED_ADDITION, issuedData->attrVals.valueGreen_Led); 
    }



    if(0x01 == issuedData->attrFlags.flagBlue_Led)
    {
        info->event[info->num] = EVENT_BLUE_LED;
        info->num++;
        dataPoints->valueBlue_Led = gizX2Y(BLUE_LED_RATIO,  BLUE_LED_ADDITION, issuedData->attrVals.valueBlue_Led); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueRgbLed_OnOff != cur->valueRgbLed_OnOff)
    {
        GIZWITS_LOG("valueRgbLed_OnOff Changed\n");
        ret = 1;
    }
    if(last->valueRed_Led != cur->valueRed_Led)
    {
        GIZWITS_LOG("valueRed_Led Changed\n");
        ret = 1;
    }
    ...
    if(last->valueBlue_Led != cur->valueBlue_Led)
    {
        GIZWITS_LOG("valueBlue_Led Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(RGBLED_ONOFF_BYTEOFFSET,RGBLED_ONOFF_BITOFFSET,RGBLED_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRgbLed_OnOff);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueRed_Led = gizY2X(RED_LED_RATIO,  RED_LED_ADDITION, dataPoints->valueRed_Led); 
    devStatusPtr->valueGreen_Led = gizY2X(GREEN_LED_RATIO,  GREEN_LED_ADDITION, dataPoints->valueGreen_Led); 
    devStatusPtr->valueBlue_Led = gizY2X(BLUE_LED_RATIO,  BLUE_LED_ADDITION, dataPoints->valueBlue_Led); 
